...
void FileCopyer::copy()
{
    ...
    while (indx < src.count())
    {
        ...
        while (fSize) {
            ...
            {
                emit finished(false);
                emit oneFinished(QFileInfo(srcFile).filePath(),QFileInfo(dstFile).filePath(),false);
                //emit copy
                return;
            }
            ...
            {
                emit oneFinished(QFileInfo(srcFile).filePath(),QFileInfo(dstFile).completeBaseName(),false);
        ...
        srcFile.close();
        dstFile.close();
        emit oneFinished(QFileInfo(srcFile).filePath(),QFileInfo(dstFile).completeBaseName(),true);
        indx++;
